import { Request, numberWithCommas, findIndexListElement, getClosestElement } from '../utils/index.js'
...
export default class ProductDetail {
  ...
  saveStorage() {
    try {
        const products_cart = this.selectedOptions.map((item) => ({
  ...
  bindEvents() {
    ...
    this.renderElement.addEventListener('input', (e) => {
      if (e.target.tagName === 'SELECT') {
        ...
        const isPossibleSelect = this.selectedOptions.find(item => item.id === Number(e.target.value));
        if (!isPossibleSelect) {
          const targetOption = productOptions.find(item => item.id === Number(e.target.value));
      ...
      if (e.target.tagName === 'INPUT') {
        if (e.target.value > 0) {
          ...
          this.selectedOptions[index].quantity = e.target.value
          replaceElement.textContent = `${this.product.name} ${targetOption.name} ${numberWithCommas((targetOption.price + this.product.price) * e.target.value)}원`
  ...
  optionToString(productName, productOption) {
    let result = ``
    ...
    result += productOption.price > 0 ? ` (+${numberWithCommas(productOption.price)})` : ''
    result += `</option>`
    return result
  }
  async render() {
    this.product = await Request(`/src/assets/products/item${this.productId}.json`);
    ...
    $detail.innerHTML = `
      ...
      <div class="ProductDetail__info">
        ...
        <select>
          <option>선택하세요.</option>
          ${productOptions.map(productOption => `${this.optionToString(name, productOption)}`)}
        </select>
